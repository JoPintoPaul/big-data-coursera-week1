...
object WikipediaRanking {
  ...
  def occurrencesOfLang(lang: String, rdd: RDD[WikipediaArticle]): Int = {
    rdd
      .filter(_.text.split(" ").contains(lang))
      .count()
      .toInt
  }
  ...
  def rankLangs(langs: List[String], rdd: RDD[WikipediaArticle]): List[(String, Int)] = {
    val unordered: List[(String, Int)] = langs map { lang =>
      (lang, occurrencesOfLang(lang, rdd))
    }
    unordered.sortBy(_._2).reverse
  }
  ...
  def rankLangsUsingIndex(index: RDD[(String, Iterable[WikipediaArticle])]): List[(String, Int)] = {
    val counted: RDD[(String, Int)] = index.map { entry =>
      val count = entry._2.toList.length
      (entry._1, count)
    }
    counted.collect().toList.sortBy(_._2).reverse
  }
